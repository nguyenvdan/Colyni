...
const DEFAULT_MARKERS = [
  ...
  { lat: 38.03, lng: -78.51, label: 'Charlottesville' },
]
...
const DEFAULT_CONNECTIONS: { from: [number, number]; to: [number, number] }[] = [
  ...
  { from: [28.61, 77.21], to: [38.03, -78.51] },
  { from: [51.51, -0.13], to: [38.03, -78.51] },
  { from: [38.03, -78.51], to: [37.78, -122.42] },
]
...
export function InteractiveGlobe({
  ...
  arcColor = 'rgba(61, 140, 94, 0.45)',
  markerColor = 'rgba(61, 140, 94, 1)',
  ...
  const draw = useCallback(() => {
    ...
    glowGrad.addColorStop(0, 'rgba(61, 140, 94, 0.04)')
    glowGrad.addColorStop(1, 'rgba(61, 140, 94, 0)')
    ctx.fillStyle = glowGrad
    ctx.fillRect(0, 0, w, h)

    ctx.beginPath()
    ctx.arc(cx, cy, radius, 0, Math.PI * 2)
    ctx.strokeStyle = 'rgba(61, 140, 94, 0.08)'
    ctx.lineWidth = 1
    ctx.stroke()
    ...
    for (const marker of markers) {
      ...
      if (marker.label) {
        ctx.font = "10px 'Geist Sans', system-ui, sans-serif"
