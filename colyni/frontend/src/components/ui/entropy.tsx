...
const BRAND_GREEN = '#3d8c5e'
const BRAND_GREEN_DIM = '#2d6b47'
...
export function Entropy({ className = '', size = 400 }: EntropyProps) {
  ...
  useEffect(() => {
    ...
    const particleColor = BRAND_GREEN
    ...
    class Particle {
      ...
      draw(ctx: CanvasRenderingContext2D) {
        const alpha = this.order ? 0.85 - this.influence * 0.5 : 0.8
      ...
      particles.forEach((particle) => {
        ...
        particle.neighbors.forEach((neighbor) => {
          ...
          if (distance < 50) {
            ...
            ctx.strokeStyle = `${BRAND_GREEN_DIM}${Math.round(alpha * 255)
              .toString(16)
              .padStart(2, '0')}`
            ctx.lineWidth = 0.5
            ctx.beginPath()
  ...
  return (
    <div
      className={`relative overflow-hidden rounded-xl bg-[#0a0a09] ${className}`}
    ...
    >
      ...
      />
      <div className="pointer-events-none absolute inset-x-0 bottom-0 h-1/3 bg-gradient-to-t from-[#0a0a09] to-transparent" />
    </div>
  )
}
