...
from colyni_cluster.shared.types.worker.downloads import FileListEntry, RepoFileDownloadProgress
...
async def temp_models_dir(tmp_path: Path) -> AsyncIterator[Path]:
    ...
    with patch("colyni_cluster.download.download_utils.COLYNI_CLUSTER_MODELS_DIR", models_dir):
        yield models_dir
...
class TestFileVerification:
    ...
    async def test_redownload_when_file_size_changes_upstream(
        ...
        with (
            patch(
                "colyni_cluster.download.download_utils.file_meta",
                new_callable=AsyncMock,
                return_value=(remote_size, remote_hash),
            ) as mock_file_meta,
            patch(
                "colyni_cluster.download.download_utils.create_http_session"
            ...
            with patch(
                "colyni_cluster.download.download_utils.calc_hash",
                new_callable=AsyncMock,
...
class TestFileListCache:
    ...
    async def test_fetch_fresh_and_update_cache(
        ...
        with (
            patch("colyni_cluster.download.download_utils.COLYNI_CLUSTER_MODELS_DIR", models_dir),
            patch(
                "colyni_cluster.download.download_utils.fetch_file_list_with_retry",
                new_callable=AsyncMock,
