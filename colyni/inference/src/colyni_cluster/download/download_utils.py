...
from colyni_cluster.download.huggingface_utils import (
    filter_repo_objects,
    get_allow_patterns,
    get_auth_headers,
    get_hf_endpoint,
...
from colyni_cluster.shared.types.worker.shards import ShardMetadata
...
def resolve_model_in_path(model_id: ModelId) -> Path | None:
    """Search COLYNI_CLUSTER_MODELS_PATH directories for a pre-existing model.
    ...
    files are present.
    """
    if COLYNI_CLUSTER_MODELS_PATH is None:
        return None
    normalized = model_id.normalize()
    for search_dir in COLYNI_CLUSTER_MODELS_PATH:
...
def build_model_path(model_id: ModelId) -> Path:
    ...
    if found is not None:
        return found
    return COLYNI_CLUSTER_MODELS_DIR / model_id.normalize()
...
async def ensure_models_dir() -> Path:
    await aios.makedirs(COLYNI_CLUSTER_MODELS_DIR, exist_ok=True)
    return COLYNI_CLUSTER_MODELS_DIR
...
async def seed_models(seed_dir: str | Path):
    """Move models from resources folder to COLYNI_CLUSTER_MODELS_DIR."""
