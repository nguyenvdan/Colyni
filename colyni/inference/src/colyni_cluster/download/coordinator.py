...
from colyni_cluster.download.download_utils import (
    RepoDownloadProgress,
    delete_model,
    map_repo_download_progress_to_download_progress_data,
    resolve_model_in_path,
)
from colyni_cluster.download.shard_downloader import ShardDownloader
from colyni_cluster.shared.constants import COLYNI_CLUSTER_MODELS_DIR, COLYNI_CLUSTER_MODELS_PATH
from colyni_cluster.shared.models.model_cards import ModelId, get_model_cards
...
from colyni_cluster.shared.types.worker.downloads import (
    DownloadCompleted,
    DownloadFailed,
    DownloadOngoing,
    DownloadPending,
...
from colyni_cluster.shared.types.worker.shards import PipelineShardMetadata, ShardMetadata
from colyni_cluster.utils.channels import Receiver, Sender
...
class DownloadCoordinator:
    ...
    def _model_dir(self, model_id: ModelId) -> str:
        return str(COLYNI_CLUSTER_MODELS_DIR / model_id.normalize())
    ...
    async def _start_download(self, shard: ShardMetadata) -> None:
        ...
        # Check COLYNI_CLUSTER_MODELS_PATH for pre-downloaded models
        found_path = resolve_model_in_path(model_id)
        if found_path is not None:
            logger.info(
                f"DownloadCoordinator: Model {model_id} found in COLYNI_CLUSTER_MODELS_PATH at {found_path}"
            )
            completed = DownloadCompleted(
    ...
    async def _delete_download(self, model_id: ModelId) -> None:
        # Protect read-only models (from COLYNI_CLUSTER_MODELS_PATH) from deletion
        if model_id in self.download_status:
            current = self.download_status[model_id]
            if isinstance(current, DownloadCompleted) and current.read_only:
                logger.warning(
                    f"Refusing to delete read-only model {model_id} (from COLYNI_CLUSTER_MODELS_PATH)"
                )
                return
    ...
    async def _emit_existing_download_progress(self) -> None:
        while True:
            try:
                ...
                # Scan COLYNI_CLUSTER_MODELS_PATH for pre-downloaded models
                if COLYNI_CLUSTER_MODELS_PATH is not None:
