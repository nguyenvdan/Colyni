...
from colyni_cluster.shared.constants import COLYNI_CLUSTER_CONFIG_FILE, COLYNI_CLUSTER_MODELS_DIR
...
from colyni_cluster.utils.channels import Sender
from colyni_cluster.utils.pydantic_ext import TaggedModel
...
class NodeConfig(TaggedModel):
    """Node configuration from COLYNI_CLUSTER_CONFIG_FILE, reloaded from the file only at startup. Other changes should come in through the API and propagate from there"""

    @classmethod
    async def gather(cls) -> Self | None:
        cfg_file = anyio.Path(COLYNI_CLUSTER_CONFIG_FILE)
    ...
    async def gather(cls) -> Self:
        return cls(
            disk_usage=await to_thread.run_sync(
                lambda: DiskUsage.from_path(COLYNI_CLUSTER_MODELS_DIR)
            )
        )
