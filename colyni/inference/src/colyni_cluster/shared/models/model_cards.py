...
from colyni_cluster.shared.constants import (
    COLYNI_CLUSTER_CUSTOM_MODEL_CARDS_DIR,
...
_custom_cards_dir = Path(str(COLYNI_CLUSTER_CUSTOM_MODEL_CARDS_DIR))
CARD_SEARCH_PATH = [
    Path(COLYNI_CLUSTER_RESOURCES_DIR) / "inference_model_cards",
    Path(COLYNI_CLUSTER_RESOURCES_DIR) / "image_model_cards",
    _custom_cards_dir,
]
...
def is_custom_card(model_id: ModelId) -> bool:
    ...
    card_path = Path(str(COLYNI_CLUSTER_CUSTOM_MODEL_CARDS_DIR)) / (
...
async def fetch_safetensors_size(model_id: ModelId) -> Memory:
    ...
    from colyni_cluster.shared.types.worker.downloads import ModelSafetensorsIndex
