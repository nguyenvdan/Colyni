...
from colyni_cluster.shared.constants import COLYNI_CLUSTER_MAX_CONCURRENT_REQUESTS
from colyni_cluster.shared.types.chunks import ErrorChunk, PrefillProgressChunk
from colyni_cluster.shared.types.common import ModelId
from colyni_cluster.shared.types.events import ChunkGenerated, Event
from colyni_cluster.shared.types.mlx import Model
from colyni_cluster.shared.types.tasks import CANCEL_ALL_TASKS, TaskId, TextGeneration
from colyni_cluster.shared.types.text_generation import TextGenerationTaskParams
from colyni_cluster.shared.types.worker.runner_response import GenerationResponse, ToolCallResponse
from colyni_cluster.utils.channels import MpReceiver, MpSender
from colyni_cluster.worker.engines.mlx.cache import KVPrefixCache
from colyni_cluster.worker.engines.mlx.generator.batch_generate import ExoBatchGenerator
from colyni_cluster.worker.engines.mlx.generator.generate import (
    PrefillCancelled,
    mlx_generate,
    warmup_inference,
)
from colyni_cluster.worker.engines.mlx.utils_mlx import (
    apply_chat_template,
    mx_all_gather_tasks,
    mx_any,
)
from colyni_cluster.worker.runner.bootstrap import logger
...
COLYNI_CLUSTER_RUNNER_MUST_FAIL = "COLYNI CLUSTER RUNNER MUST FAIL"
COLYNI_CLUSTER_RUNNER_MUST_OOM = "COLYNI CLUSTER RUNNER MUST OOM"
COLYNI_CLUSTER_RUNNER_MUST_TIMEOUT = "COLYNI CLUSTER RUNNER MUST TIMEOUT"


def _check_for_debug_prompts(task_params: TextGenerationTaskParams) -> None:
    """Check for debug prompt triggers in the input."""
    from colyni_cluster.worker.engines.mlx.utils_mlx import mlx_force_oom
    ...
    if not prompt:
        return
    if COLYNI_CLUSTER_RUNNER_MUST_FAIL in prompt:
        raise Exception("Artificial runner exception - for testing purposes only.")
    if COLYNI_CLUSTER_RUNNER_MUST_OOM in prompt:
        mlx_force_oom()
    if COLYNI_CLUSTER_RUNNER_MUST_TIMEOUT in prompt:
        time.sleep(100)
...
class BatchGenerator(InferenceGenerator):
    ...
    def step(
        ...
        while self._queue and len(self._active_tasks) < COLYNI_CLUSTER_MAX_CONCURRENT_REQUESTS:
            task = self._queue.popleft()
            try:
