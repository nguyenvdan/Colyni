...
from colyni_cluster.worker.engines.mlx.constants import TRUST_REMOTE_CODE
...
from colyni_cluster.download.download_utils import build_model_path
from colyni_cluster.shared.types.common import Host
from colyni_cluster.shared.types.memory import Memory
from colyni_cluster.shared.types.mlx import Model
from colyni_cluster.shared.types.tasks import TaskId, TextGeneration
...
from colyni_cluster.worker.engines.mlx.auto_parallel import (
    LayerLoadedCallback,
    TimeoutCallback,
    eval_with_timeout,
    get_inner_model,
...
def shard_and_load(
    ...
    base_timeout = float(
        os.environ.get(
            "COLYNI_CLUSTER_MODEL_LOAD_TIMEOUT",
            os.environ.get("EXO_MODEL_LOAD_TIMEOUT", "300"),
        )
    )
...
def apply_chat_template(
    ...
    if _needs_dsml_encoding(task_params):
        from colyni_cluster.worker.engines.mlx.dsml_encoding import encode_messages
