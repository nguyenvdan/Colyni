...
from colyni_cluster.shared.models.model_cards import ModelCard, ModelTask
...
MODEL_PATH = COLYNI_CLUSTER_MODELS_DIR / "mlx-community--gpt-oss-20b-MXFP4-Q8"
TOTAL_LAYERS = 24
MAX_TOKENS = 10
SEED = 42
TEMPERATURE = 0.0
...
def _build_prompt(tokenizer: Any, prompt_tokens: int) -> tuple[str, Any]:
    ...
    from colyni_cluster.worker.engines.mlx.utils_mlx import apply_chat_template
...
def _run_single_device(
    ...
    try:
        ...
        from colyni_cluster.shared.types.worker.shards import PipelineShardMetadata
        from colyni_cluster.worker.engines.mlx.cache import encode_prompt
        from colyni_cluster.worker.engines.mlx.generator.generate import mlx_generate
...
def _run_pipeline_device(
    ...
    try:
        ...
        from colyni_cluster.worker.engines.mlx.cache import encode_prompt
        from colyni_cluster.worker.engines.mlx.generator.generate import mlx_generate
        from colyni_cluster.worker.engines.mlx.utils_mlx import shard_and_load
        ...
        def distributed_prompt_progress_callback(_group: Any = group) -> None:
            from colyni_cluster.worker.engines.mlx.utils_mlx import mx_any
