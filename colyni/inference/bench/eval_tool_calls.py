...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Multi-API tool-calling eval for Colyni cluster",
...
""",
    ...
    cluster_client = ClusterClient(args.host, args.port, timeout_s=args.timeout)
    _short_id, full_model_id = resolve_model_short_id(cluster_client, args.model)

    selected = settle_and_fetch_placements(
        cluster_client, full_model_id, args, settle_timeout=args.settle_timeout
    )
    if not selected:
    ...
    cluster_client.request_json("POST", "/instance", body={"instance": instance})
    try:
        wait_for_instance_ready(cluster_client, instance_id)
    except (RuntimeError, TimeoutError) as e:
        print(f"Failed to initialize placement: {e}", file=sys.stderr)
        with contextlib.suppress(ClusterHttpError):
            cluster_client.request_json("DELETE", f"/instance/{instance_id}")
        sys.exit(1)
    ...
    try:
        ...
        try:
            ...
            if e.status != 404:
                raise
        wait_for_instance_gone(cluster_client, instance_id)
