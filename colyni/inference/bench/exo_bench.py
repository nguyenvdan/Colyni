# type: ignore
#!/usr/bin/env python3
"""Tool-calling eval for Colyni cluster OpenAI-compatible API.
...
Start colyni-cluster with a model first, then run:
...
def run_one_completion(
    client: ClusterClient, model_id: str, pp_hint: int, tg: int, prompt_sizer: PromptSizer
...
def main() -> int:
    ap = argparse.ArgumentParser(
        prog="colyni-cluster-bench",
        description="Benchmark Colyni cluster model throughput across placement previews.",
    )
    ...
    client = ClusterClient(args.host, args.port, timeout_s=args.timeout)
    ...
    if tokenizer is None:
        raise RuntimeError("[colyni-cluster-bench] tokenizer load failed")

    try:
        prompt_sizer = PromptSizer(tokenizer)
        logger.debug(f"[colyni-cluster-bench] loaded tokenizer: {full_model_id} for prompt sizer")
    except Exception:
        logger.error("[colyni-cluster-bench] tokenizer usable but prompt sizing failed")
        raise
    ...
    logger.debug(f"colyni-cluster-bench model: short_id={short_id} full_id={full_model_id}")
    ...
    for preview in selected:
        ...
        try:
            ...
            with contextlib.suppress(ClusterHttpError):
                ...
                for concurrency in concurrency_list:
                    ...
                    for r in range(args.repeat):
                        ...
                        if concurrency <= 1:
                            ...
                            # Each thread gets its own ClusterClient (separate HTTP connection)
