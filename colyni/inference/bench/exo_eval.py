# type: ignore
#!/usr/bin/env python3
"""Quality evaluation for Colyni cluster — matches Artificial Analysis methodology.

Runs LLM benchmarks against the Colyni cluster's OpenAI-compatible API using the same
...
def main() -> int:
    ap = argparse.ArgumentParser(
        prog="colyni-cluster-eval",
        description="Quality evaluation for Colyni cluster — matches Artificial Analysis methodology.",
    )
    ...
    ap.add_argument(
        "--skip-instance-setup",
        action="store_true",
        help="Skip cluster instance management (assumes model is already running).",
    )
