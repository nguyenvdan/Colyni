...
class ClusterClient:
    ...
    def request_json(
        ...
        try:
            ...
            if resp.status >= 400:
                raise ClusterHttpError(resp.status, resp.reason, text[:300])
...
def wait_for_instance_ready(
    client: ClusterClient, instance_id: str, timeout: float = 24000.0
) -> None:
...
def wait_for_instance_gone(
    client: ClusterClient, instance_id: str, timeout: float = 3.0
) -> None:
    start_time = time.time()
    while time.time() - start_time < timeout:
        try:
...
def resolve_model_short_id(
    client: ClusterClient, model_arg: str, *, force_download: bool = False
) -> tuple[str, str]:
...
def fetch_and_filter_placements(
    client: ClusterClient, full_model_id: str, args: argparse.Namespace
) -> list[dict[str, Any]]:
...
def run_planning_phase(
    ...
    for node_id in node_ids:
        ...
        # Delete from smallest to largest (skip read-only models from COLYNI_CLUSTER_MODELS_PATH)
        completed = [
            (
...
def add_common_instance_args(ap: argparse.ArgumentParser) -> None:
    ap.add_argument(
        ...
        default=os.environ.get(
            "COLYNI_CLUSTER_HOST", os.environ.get("EXO_HOST", "localhost")
        ),
    )
    ap.add_argument(
        "--port",
        type=int,
        default=int(
            os.environ.get("COLYNI_CLUSTER_PORT", os.environ.get("EXO_PORT", "52415"))
        ),
    )
    ...
    ap.add_argument(
        "--force-download",
        action="store_true",
        help="If model not in /models, add it from HuggingFace via the cluster and download.",
    )
    ap.add_argument(
        "--max-nodes",
        type=int,
